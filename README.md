...
Saya mengganti fungsi `ThreadPool::new` dengan `ThreadPool::build`. Perubahan ini dilakukan untuk mengikuti konvensi dan *best practice* dalam bahasa Rust. Sebelumnya, fungsi `new` menggunakan `assert!(size > 0)` yang akan langsung memicu program berhenti paksa apabila caller memasukkan angka 0. Padahal, seharusnya fungsi bernama `new` seharusnya tidak boleh membuat program mati mendadak seperti itu. Untuk proses inisialisasi yang berpotensi gagal (seperti mengalokasikan 0 thread), lebih aman menggunakan fungsi `build` yang mengembalikan tipe `Result<T, E>`. Dengan bagitu, program tidak akan langsung *crash*, tetapi fungsi `build` akan mengembalikan `Err(PoolCreationError)` dahulu. Hal ini memungkinkan fungsi  `main` untuk menangkap error dan memberikan pesan error yang rapi di layar, misalnya menggunakan metode `unwrap_or_else`.
